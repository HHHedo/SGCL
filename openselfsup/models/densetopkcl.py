...
class DenseTopkCL(nn.Module):
    ...
    def __init__(self,
                 ...
                 T=0.07,
                 **kwargs):
        super(DenseTopkCL, self).__init__()
    ...
    def forward_train(self, img, **kwargs):
        ...
        dense_k = 3
        densecl_topk_ind = torch.topk(backbone_sim_matrix, k=dense_k+1, dim=2)[1][:,:, 1:] # NxS^2xk

        topk_indexed_k_grid = torch.gather(k_grid.unsqueeze(-1).expand(-1, -1, -1, dense_k), 2, densecl_topk_ind.unsqueeze(1).expand(-1, k_grid.size(1), -1, -1)) # NxCxS^2xk
        densecl_topk_q = (q_grid.unsqueeze(-1).expand(-1, -1, -1, dense_k) * topk_indexed_k_grid).sum(1) #similarity NxS^2xk
        # l_pos_topk_dense = densecl_topk_q.view(-1).unsqueeze(-1) # NS^2X1
        den_topk_s  = torch.sum(torch.exp(densecl_topk_q/self.T), dim=2).view(-1).unsqueeze(-1)
        ...
        l_neg_dense_hard, _ = torch.topk(l_neg_dense, k=4096, sorted=True, dim=1)
        neg_dense_s = torch.sum(torch.exp(l_neg_dense_hard/self.T), dim=1, keepdim=True)
        loss_single = self.head(l_pos, l_neg)['loss_contra']
        loss_dense = self.head(l_pos_dense, l_neg_dense)['loss_contra']
        loss_dense_topk = -torch.mean(torch.log(den_topk_s/(den_topk_s + neg_dense_s) + 1e-7))
        losses = dict()
        ...
        losses['loss_contra_dense_topk'] = loss_dense_topk * self.loss_lambda 
